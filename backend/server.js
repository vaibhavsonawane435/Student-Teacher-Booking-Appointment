...
// Connect to MongoDB
mongoose.connect('mongodb://127.0.0.1:27017/appointment_db')
...
app.post('/api/admin/add-teacher', async (req, res) => {
  ...
  const teacher = new User({ name, email, password, department, subject, role: 'teacher', isApproved: true });
  await teacher.save();
...
app.get('/api/teacher/appointments/:teacherId', async (req, res) => {
  const apps = await Appointment.find({ teacherId: req.params.teacherId }).populate('studentId', 'name');
  res.json(apps);
});
...
app.get('/api/admin/appointments', async (req, res) => {
  const apps = await Appointment.find().populate('studentId', 'name email').populate('teacherId', 'name department subject');
  res.json(apps);
});
...
app.get('/api/student/appointments/:studentId', async (req, res) => {
  const apps = await Appointment.find({ studentId: req.params.studentId }).populate('teacherId', 'name department subject');
  res.json(apps);
});
...
app.post('/api/book', async (req, res) => {
  ...
  const appt = new Appointment({ studentId, teacherId, date, time, purpose, message });
  await appt.save();
...
app.get('/api/messages/:userId', async (req, res) => {
  ...
  const msgs = await Message.find({ $or: [{ senderId: uid }, { receiverId: uid }] })
